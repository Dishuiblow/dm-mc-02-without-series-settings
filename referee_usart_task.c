...
uint8_t usart10_buf[2][USART_RX_BUF_LENGHT];
...
void referee_usart_task(void const * argument)
{
    ...
    usart10_init(usart10_buf[0], usart10_buf[1], USART_RX_BUF_LENGHT);
	  // 调取裁判系统接收数据指针
...
void USART10_IRQHandler(void)
{
    ...
    if(USART10->SR & UART_FLAG_IDLE)
    {
        __HAL_UART_CLEAR_PEFLAG(&huart10);

        static uint16_t this_time_rx_len = 0;

        if ((huart10.hdmarx->Instance->CR & DMA_SxCR_CT) == RESET)
        {
            ...
            huart10.hdmarx->Instance->CR |= DMA_SxCR_CT;
            __HAL_DMA_ENABLE(huart10.hdmarx);
            fifo_s_puts(&referee_fifo, (char*)usart10_buf[0], this_time_rx_len);
        ...
        else
        {
            ...
            huart10.hdmarx->Instance->CR &= ~(DMA_SxCR_CT);
            __HAL_DMA_ENABLE(huart10.hdmarx);
            fifo_s_puts(&referee_fifo, (char*)usart10_buf[1], this_time_rx_len);
						// 未进中断该函数就会停止计时，用以判断设备离线
            detect_hook(REFEREE_TOE);
        }
    }
