...
void detect_task(void const *pvParameters)
{
    ...
    while (1)
    {
        static uint8_t error_num_display = 0;				// 当前处理优先级最高的设备ID码
        system_time = xTaskGetTickCount();

        error_num_display = ERROR_LIST_LENGHT;
				// 因为最后一个设备是空的，这里强行设置在线。属于bug
...
//	return (error_list[toe].error_exist == 1);
    return (error_list[toe].is_lost == 1);
}


/**
  * @brief          记录当前设备时间，以记录的new和last来判断设备是否离线。同时进行数据错误检测和修复
  * @param[in]      toe:设备
  * @retval         none
  */
void detect_hook(uint8_t toe)
{
    ...
    {
//        error_list[toe].is_lost = 0;				// 检测到掉线后清0，等待set_offline_time后再次检测
...
/**
  ...
  * @retval         error_list的指针
  */
const error_t *get_REF_error_list_point(void)
{
    return &error_list[9];
}
...
/* [3]: {set_offline_time, set_online_time,  priority}*/
static void detect_init(uint32_t time)
{
    //设置 {离线时间，上线稳定工作时间，优先级} {offlineTime onlinetime priority}
    uint16_t set_item[ERROR_LIST_LENGHT][3] =
        {
            {30, 40, 15},   //SBUS
            {10, 10, 11},   //motor1
    ...
    for (uint8_t i = 0; i < ERROR_LIST_LENGHT; i++)
    {
        ...
        error_list[i].data_is_error_fun = NULL;						// 判断数据是否出错
